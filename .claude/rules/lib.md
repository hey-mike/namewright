...
- HS256 JWT, 7d expiry — must match KV TTL and cookie Max-Age (see `.claude/rules/contracts.md`)
- `verifySession` returns null on any failure — never throws

## kv.ts

- TTL is 7d (604800s) — long enough for sleep-on-it / share-with-cofounder before commit
- `NONCE_TTL_SECONDS` is deliberately shorter (24h) — single-use; do not bump to match
